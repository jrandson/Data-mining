...
class Classifier():
   ...
   def __init__(self,filename):
      ...
      self.medianAndDeviation = self.getMedianAndDeviation()
   ...
   def getMedianAndDeviation(self):
   ...
   def normalizeVector(self, v):
      ...
      for i in range(len(vector)):
         (median, asd) = self.getMedianAndDeviation()
...
values = [10,23,45,56,57,103]

def change_scale(values,minimo = 0,maximo = 1):

   new_scale = [ minimo + (maximo - minimo)*i/float(max(values)) for i in values]
   print new_scale
   print ''
   scale2 = [ minimo + (maximo-minimo)*(i - min(values))/float(max(values) - min(values)) for i in values]
   print scale2
   print ''

change_scale(values,-1,1)
